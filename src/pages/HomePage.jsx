    ...
    <main className="min-h-[100dvh] bg-[radial-gradient(circle_at_top,_#1b2439_0%,_#0a0d15_50%,_#080a10_100%)] px-4 py-7 text-white md:px-8">
      <div className="mx-auto grid w-full max-w-[1200px] gap-8 lg:grid-cols-[1fr_1.15fr] lg:items-center">
        <section className="space-y-5">
          ...
          <h1 className="font-title text-5xl leading-[1.03] md:text-6xl">Tinder Food devient une application complète.</h1>
          <p className="max-w-[38rem] text-base text-white/80">
            Découvrez des plats d'exception autour de vous, balayez avec fluidité et retrouvez vos coups de cœur en
            quelques secondes.
          </p>
          ...
          <div className="grid max-w-[32rem] grid-cols-3 gap-3 text-center text-xs text-white/70">
            <div className="rounded-xl border border-white/15 bg-white/5 px-2 py-3">Geste de balayage naturel</div>
            <div className="rounded-xl border border-white/15 bg-white/5 px-2 py-3">Favoris centralisés</div>
        ...
        <section className="mx-auto w-full max-w-[480px] rounded-[2rem] border border-white/15 bg-[#0e1320]/85 p-5 shadow-2xl shadow-black/45">
          <div className="rounded-[1.6rem] border border-white/10 bg-black p-3">
            <img alt="Aperçu de l'accueil Tinder Food" className="h-[34rem] w-full rounded-[1.2rem] object-cover" src={onboardingHero} />
          </div>
        </section>
      </div>
      ...
      <section className="mx-auto mt-12 w-full max-w-[1200px]" id="preview">
        <div className="overflow-hidden rounded-3xl border border-white/15 bg-[#0d111b] p-4 md:p-6">
          <p className="mb-4 text-xs font-semibold uppercase tracking-[0.14em] text-white/65">Référence des maquettes Tinder</p>
          <img alt="Parcours complet Tinder Food" className="w-full rounded-2xl object-cover" src={flowMock} />
        </div>
      </section>
    </main>
  );
